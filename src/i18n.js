...
recInnovation: (val) => `<strong style="color:var(--purple)">Innovation Lever:</strong> Debt backlog is blocking roadmap. Resolving it unlocks <strong>${PDE.formatCompactCurrency(val)}</strong> in project margin.`,
                ...
                methodologyMcBody:            'When enabled, runs 1,000 iterations of the full model with perturbed inputs. Each input (except riskLevel which is discrete 1–5) is sampled from a triangular distribution centred on the user\'s value with ±15% spread. MTTR uses a log-normal distribution (Box-Muller transform) with ±25% spread, reflecting the heavy-tailed nature of incident recovery times. Outputs are reported as median with 90% confidence interval [p5, p95]. The LCG PRNG draws a fresh pseudo-random seed once per browser session (shown next to the simulation progress); the re-roll control draws an independent fresh sample. Disabled by default.',
