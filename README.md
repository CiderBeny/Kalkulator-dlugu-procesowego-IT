...
### Kluczowe Funkcje
* **Kalkulacja Kosztu Chaosu:** OPEX waste, Risk Exposure, Opportunity Loss, Net Debt, Payback Period — wyliczenia oparte na 11 pytaniach diagnostycznych (Q1–Q11: Manual Effort, Lead Time, Documentation Standard, Downtime Cost, Human Errors, itd.).
...
### Key Features
* **Chaos Cost Calculation:** OPEX waste, Risk Exposure, Opportunity Loss, Net Debt, Payback Period — based on 11 diagnostic questions (Q1–Q11: Manual Effort, Lead Time, Documentation Standard, Downtime Cost, Human Errors, etc.).
...
* Uwaga o bezpieczeństwie / Security note: GitHub Pages nie pozwala ustawić nagłówków HTTP, więc klikjackingu nie da się w pełni zablokować samą warstwą CDN. `frame-ancestors 'none'` jest zadeklarowane w meta-CSP oraz w nagłówku `Content-Security-Policy` dev-servera (gdzie jest w pełni egzekwowane); w `<meta>` przeglądarki ignorują tę dyrektywę, dlatego dodatkową best-effort ochroną na GitHub Pages jest framebusting JS w `src/font-bootstrap.js`. Pełne nagłówki `X-Frame-Options: DENY`/HSTS wymagają warstwy Cloudflare/Netlify. / Security note: GitHub Pages does not allow custom HTTP headers, so clickjacking cannot be fully blocked at the CDN layer alone. `frame-ancestors 'none'` is declared both in the meta CSP and in the dev-server `Content-Security-Policy` header (where it is fully enforced); browsers ignore the directive in a `<meta>` tag, so the additional best-effort protection on GitHub Pages is the JS framebusting guard in `src/font-bootstrap.js`. Full `X-Frame-Options: DENY`/HSTS headers require a Cloudflare/Netlify layer.
